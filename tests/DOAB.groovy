...
config = null;
cfg_file = new File('./sync-doab-config.json')
if ( cfg_file.exists() ) {
...
println("NBK Ingest Phase 1 Adapter for DOAB");
...
def addRecord(recordid, raw, htable) {
  ...
  try {
    ...
    p.add( Bytes.toBytes("nbk"), Bytes.toBytes("sourceid"), Bytes.toBytes("DOAB") )
...
def pullLatest(config, cfg_file) {
  ...
  // http://www.doabooks.org/oai?verb=ListRecords&from=2012-01-01&metadataPrefix=marcxml
  doSync('http://www.doabooks.org','/oai', 'marcxml', 'doab', cursor, config, cfg_file, htable) { r ->
