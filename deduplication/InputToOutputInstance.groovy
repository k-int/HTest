...
job.setJarByClass(InputToOutputInstance.class);
...
public class MapInputToOutputMapper extends TableMapper<ImmutableBytesWritable, Put> {
    ...
    public static byte[] INSTANCE_COL = Bytes.toBytes('instance_hash')
    ...
    public void map(ImmutableBytesWritable row, Result value, Context context) throws IOException, InterruptedException {
        ...
        byte[] instance_bytes = value.getValue(NBK_FAMILY, INSTANCE_COL)
        if (instance_bytes) instance = new String(instance_bytes);
        ...
        if (recsyn?.equals('mods') && (recwork != null)) {
            ...
            Put put = new Put(work_bytes);
            put.add(Bytes.toBytes("nbk"), Bytes.toBytes("title"), Bytes.toBytes(title_hash_str));
            put.add(Bytes.toBytes("nbk"), Bytes.toBytes("instance"), Bytes.toBytes(instance));
